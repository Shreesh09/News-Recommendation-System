...
const addNewsRead = async (username='test2', news_id) => {
  try {
...
const NewsDashboard = () => {
  ...
  return (
    <>
    <ResponsiveAppBar state={state} search={search}></ResponsiveAppBar>
        ...
        {(!articles || !articles.length || articles.length == 0?"No results":articles.map((article, index) => (
          <Grid item key={index} xs={12} sm={6} md={4}>
            <Card sx={{ width: '90%', height: '230px', border: "1px solid #CFD3D8", boxShadow: "1px 5px 10px rgb(100, 100, 100, .2)"}}>
              <CardActionArea onClick={()=>addNewsRead('test2',article.id)} href={article.link} target="_blank" rel="noopener noreferrer">
                <CardContent>
